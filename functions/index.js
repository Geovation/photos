...
async function hostMetadata(req, res) {
  ...
  if (photo && photo.exists && photo.data().published) {
    const TW_DESCRIPTION = config.metadata.twDescriptionField ? photo.data()[config.metadata.twDescriptionField] : config.metadata.twDescription;    const TW_TITLE = config.metadata.twTitle;
